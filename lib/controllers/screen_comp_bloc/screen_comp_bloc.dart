...
class ScreenCompBloc extends Bloc<CompEvent, ScreenCompState>{
  ...
  Future<void> _screenComp(LoadScreenComp event, Emitter<ScreenCompState> emit) async{
    emit(LoadingScreenComp());
    try{
      final stream = _fireStoreServices.streamCollection(path: mobilesPath, builder: (map) => Mobile.fromJson(map),
      query: (query) {
      ...
      await emit.forEach(stream, onData: (List<Mobile> mobiles){
        List<MobileTheme> mobilesTheme  = _filtrationHelper.getScreensCompatibilities(mobiles: mobiles, mobile: event.mobile);
