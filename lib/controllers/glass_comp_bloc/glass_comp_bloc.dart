...
class GlassCompBloc extends Bloc<CompEvent, GlassCompState>{
  ...
  Future<void> _glassComp(LoadGlassComp event, Emitter<GlassCompState> emit) async{
    ...
    try{
      ...
      await emit.forEach<List<Mobile>>(stream, onData: (List<Mobile> mobiles){
        List<MobileTheme> mobilesTheme = _filterationHelper.getGlassCompatibilities(mobiles: mobiles, mobile: event.mobile);
