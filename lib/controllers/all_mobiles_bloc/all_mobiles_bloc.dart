...
class AllMobilesBloc extends Bloc<LoadAllMobilesEvent, AllMobilesState>{
  ...
  void _loadAllMobiles(LoadAllMobilesEvent event, Emitter<AllMobilesState> emit) async{
    emit(LoadingAllMobiles());
    try{
      final stream = _firebaseFirestore.streamCollection(path: mobilesPath, builder: (map){
        return Mobile.fromJson(map);
      });
      await emit.forEach<List<Mobile>>(stream, onData: (List<Mobile> data){
        List<MobileTheme> mobilesTheme = _filtrationHelper.getMobilesWithTheme(mobiles: data);
